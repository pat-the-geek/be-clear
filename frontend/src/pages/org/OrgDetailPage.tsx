...
import { ArrowLeft, Edit, CalendarDays, RefreshCw, Hash, Trash2, FileOutput, FileJson, ChevronDown, Plus, Pencil, X, CheckCircle2, Loader2, CalendarClock, List } from 'lucide-react'
import { orgApi, rptApi, graphApi, exportApi } from '@/services/api'
...
export default function OrgDetailPage() {
  ...
  const { mutate: exportJson, isPending: isExporting } = useMutation({
    mutationFn: async () => {
      const res = await exportApi.org(orgId)
      downloadBlobResponse(res, `export_org_${orgId}.json`)
    },
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-stretch gap-5 mb-8">
        <EntityAvatar type="org" nom={org.obj.nom} image={imagePrincipale} size="xl" />
        ...
        <div className="flex-1 min-w-0 flex flex-col min-h-[6rem]">
          ...
          <div className="mt-auto pt-3">
            <h1 className="text-2xl font-bold text-gray-900 leading-tight break-words">{org.obj.nom}</h1>
